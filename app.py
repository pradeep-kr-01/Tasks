...
is_local = platform.system() in ['Linux', 'Darwin', 'Windows']
...
# Call the function to open the camera when the application starts
open_camera()
...
def capture_image():
    ...
    if video_capture is None:
        return jsonify({"message": "Camera is not accessible."}), 500
...
def open_camera_endpoint():
    """Open the camera (for internal use)."""
    open_camera()  # Try to open the camera
    if video_capture is None:
        return jsonify({"message": "Camera is not accessible."}), 500
    return jsonify({"message": "Camera is opened."}), 200
